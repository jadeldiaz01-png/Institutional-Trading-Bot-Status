...
def scope(dataset_id: str = "ar-tf-binance-spot-usdt-1d-v1d2"):
    return {
        "strategy_id": "AR-TF-v1",
        "asset_class": "CRYPTO_SPOT",
        "venue": "BINANCE_SPOT",
        "market": "USDT_POINT_IN_TIME_UNIVERSE",
        "timeframe": "1D",
...
def frozen_dataset(*, verified: bool = True):
    return {
        ...
        "code_sha": "c" * 40,
        "certificate_file_sha256": "f" * 64,
        "source_artifact_digest": "sha256:" + "9" * 64,
...
def scope_sha(commit: str = "b" * 40):
    return evaluate_readiness(source_commit_sha=commit, scope=scope()).manifest["scope"]["scope_sha256"]


def pass_domain(name: str, commit: str, bound_scope_sha: str):
    digest = "c" * 64
    return {
        "status": "PASS",
        "items": [{
            ...
            "scope_sha256": bound_scope_sha,
            "evidence_class": "SAME_COMMIT",
            "provenance": {
                "required": True,
                "verified": True,
...
def test_missing_evidence_and_unbound_scope_is_no_go_and_all_gates_blocked():
    r = evaluate_readiness(source_commit_sha="b" * 40)
    assert r.manifest["decision"] == "NO_GO"
    assert r.manifest["scope"]["bound"] is False
...
def test_dataset_requires_external_byte_verification():
    r = evaluate_readiness(source_commit_sha="b" * 40, scope=scope(), dataset_certificate=frozen_dataset(verified=False))
...
def test_dataset_requires_all_freeze_bindings_and_zero_unresolved():
    ...
    r = evaluate_readiness(source_commit_sha="b" * 40, scope=scope(), dataset_certificate=cert)
    gates = {g["id"]: g for g in r.manifest["gates"]}
    assert r.manifest["evidence"]["dataset"]["status"] == "FAIL"
    assert gates["G1"]["status"] == "FAIL"
    assert gates["G3"]["status"] == "FAIL"
...
def test_dataset_scope_mismatch_fails_closed():
    r = evaluate_readiness(
        source_commit_sha="b" * 40,
        scope=scope(dataset_id="fx-usdjpy-m15-v1"),
...
def test_frozen_dataset_passes_only_g1_g3_not_full_data_certification():
    r = evaluate_readiness(source_commit_sha="b" * 40, scope=scope(), dataset_certificate=frozen_dataset())
    gates = {g["id"]: g for g in r.manifest["gates"]}
    ...
    assert r.manifest["certifications"]["DATA_VERIFIED"] is False
    item = r.manifest["evidence"]["dataset"]["items"][0]
    assert item["sha256"] == "f" * 64
    assert item["source_commit_sha"] == "c" * 40
    assert item["cross_commit_binding"]["consumer_commit_sha"] == "b" * 40


def test_self_declared_pass_without_immutable_evidence_fails():
    commit = "b" * 40
    ev = {d: {"status": "PASS", "items": []} for d in REQUIRED_DOMAINS}
    r = evaluate_readiness(source_commit_sha=commit, scope=scope(), dataset_certificate=frozen_dataset(), evidence=ev)
...
def test_commit_mismatch_invalidates_same_commit_domain():
    commit = "b" * 40
    ssha = scope_sha(commit)
    ev = {d: pass_domain(d, commit, ssha) for d in REQUIRED_DOMAINS}
...
def test_scope_mismatch_invalidates_domain():
    commit = "b" * 40
    ssha = scope_sha(commit)
    ev = {d: pass_domain(d, commit, ssha) for d in REQUIRED_DOMAINS}
    ev["execution"]["items"][0]["scope_sha256"] = "0" * 64
    r = evaluate_readiness(source_commit_sha=commit, scope=scope(), dataset_certificate=frozen_dataset(), evidence=ev)
    assert r.manifest["evidence"]["execution"]["status"] == "FAIL"
    assert "item_0:scope_mismatch" in r.manifest["evidence"]["execution"]["validation_reasons"]
...
def test_all_pass_gates_without_bound_scope_still_no_go():
...
def test_full_verified_domains_and_gates_can_certify_but_never_authorize_capital():
    ...
    ssha = scope_sha(commit)
    ev = {d: pass_domain(d, commit, ssha) for d in REQUIRED_DOMAINS}
    r = evaluate_readiness(source_commit_sha=commit, scope=scope(), dataset_certificate=frozen_dataset(), evidence=ev, gates=pass_gates())
    ...
    assert r.manifest["stage"] == "SCALED_LIVE"
    assert not any(r.manifest["authorizations"].values())
...
def test_holdout_closed_by_default_and_identities_explicit():
    ...
    assert r.manifest["identities"]["dataset_producer_commit_sha"] == "c" * 40
    assert "strategy_config_sha256" in r.manifest["identities"]


def test_semantic_snapshot_is_stable_across_generation_timestamps():
    a = evaluate_readiness(source_commit_sha="b" * 40, scope=scope()).manifest
    b = evaluate_readiness(source_commit_sha="b" * 40, scope=scope()).manifest
    assert a["semantic_snapshot_sha256"] == b["semantic_snapshot_sha256"]
...
def test_generated_manifest_matches_schema():
    ...
    manifest = evaluate_readiness(source_commit_sha="b" * 40, scope=scope()).manifest
